      ...
      <div className="relative">
        ...
        <div className="relative bg-[#0A1628]/95 backdrop-blur-xl rounded-3xl border border-blue-500/10 shadow-2xl shadow-black/20">
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-3 gap-4">
              {statCards.map((stat, index) => (
                ...
                >
                  ...
                  <div className="p-4 rounded-2xl bg-white/[0.02] border border-white/[0.05] hover:border-white/[0.08] transition-all duration-300">
                    <div className="flex items-center gap-3 mb-3">
                      <motion.div
                        whileHover={{ rotate: 15 }}
                        className="p-2 rounded-xl bg-white/[0.03] border border-white/[0.05]"
                      >
                        <stat.icon className={`h-4 w-4 ${stat.textColor}`} />
                      </motion.div>
                      <span className="text-sm text-blue-200/70">{stat.title}</span>
                    </div>
            ...
            <motion.div
              variants={itemVariants}
              className="bg-white/[0.02] rounded-2xl border border-white/[0.05] p-6"
            >
              ...
              <div className="mt-4 grid grid-cols-3 gap-4">
                {balanceData.map((item, index) => (
                  <div key={item.name} className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-blue-200/60">{item.name}</span>
                  </div>
                ))}
              </div>
            </motion.div>

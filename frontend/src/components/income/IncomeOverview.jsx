...
const IncomeOverview = ({ transactions, onAddIncome }) => {
  ...
  const renderStatCard = (label, value, unit = '', isPositive = true) => (
    <div className="bg-white/5 rounded-2xl p-4 backdrop-blur-sm">
      ...
      <span className={`text-sm ${isPositive ? 'text-emerald-400' : 'text-red-400'}`}>{unit}</span>
    </div>
  );
  ...
  return (
    <div className="bg-gradient-to-br from-gray-900 to-gray-800 p-8 rounded-3xl shadow-2xl border border-gray-700/50">
      <div className="flex items-start justify-between">
        ...
        <button 
          onClick={onAddIncome}
          className="flex items-center gap-2 px-5 py-3 bg-gradient-to-r from-blue-500 to-blue-700 hover:from-blue-600 hover:to-blue-800 text-white font-medium rounded-2xl transition-all duration-200 shadow-lg shadow-blue-500/20 hover:shadow-blue-500/30 hover:scale-105"
        >
      ...
      <div className="mt-8">
        {chartData.length > 0 ? (
          <div className="space-y-6">
            ...
            <div className="bg-white/5 rounded-2xl p-6 h-[300px] backdrop-blur-sm">
              <CustomBarChart data={chartData} />
            </div>
          </div>
        ) : (
          <div className="flex flex-col items-center justify-center py-16 rounded-2xl border border-gray-700 bg-white/5 backdrop-blur-sm">

...
const ExpenseList = ({ transactions, onDelete, onDownload }) => {
  const handleDelete = async (id) => {
    try {
      ...
      onDelete(id); // Call parent function to update state
    } catch (error) {
  ...
  return (
    <div className="w-full mx-auto">
      <div className="bg-gradient-to-br from-gray-900 to-gray-800 p-8 rounded-3xl shadow-2xl border border-gray-700/50 w-full">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h5 className="text-2xl font-bold text-white mb-2">Expense Records</h5>
...
const TransactionCard = ({ title, amount, date, onDelete }) => (
  <div className="bg-gray-800 rounded-2xl p-6 transform transition-all duration-300 hover:scale-[1.02] hover:shadow-2xl w-full">
    <div className="flex items-start justify-between">
      <div className="flex items-center gap-3">
        <div className="p-3 bg-blue-500/20 rounded-xl">
          <LuArrowDownCircle className="text-blue-400 w-6 h-6" />
        </div>
        <div>
    ...
    <div className="mt-4 flex items-center gap-2">
      <LuArrowDownCircle className="text-blue-400 w-5 h-5" />

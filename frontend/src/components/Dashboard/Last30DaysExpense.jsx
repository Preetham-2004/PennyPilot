...
const Last30DaysExpense = ({ data = [] }) => {
  ...
  return (
    <motion.div
      ...
      initial="hidden"
      animate="visible"
      className="w-full md:w-[640px] h-[590px] p-1 rounded-3xl shadow-lg"
    >
          ...
          <div className="p-6 border-b border-blue-500/10">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-blue-300/50">Track your spending activity</p>
              </div>
            </div>
          </div>

          <div className="p-6 space-y-6 h-[80%]">
            {/* Filter Button */}
            <motion.div
              variants={itemVariants}
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              className="flex items-center gap-2 w-fit px-4 py-2 rounded-xl bg-white/[0.02] hover:bg-white/[0.04] border border-white/[0.05] text-blue-400 transition-all duration-300 cursor-pointer"
            >
            ...
            {chartData.length > 0 ? (
              <motion.div
                variants={itemVariants}
                className="bg-white/[0.02] rounded-2xl border border-white/[0.05] p-6 h-[80%]"
              >
